...
fn helper_build_3x3<T: Testable>() -> BidiGrowVec<T> {
    let mut v = BidiGrowVec::<T>::new();
...
fn helper_build_5x3<T: Testable>() -> BidiGrowVec<T> {
    bidigrowvec! {
...
fn helper_build_4x5<T: Testable>() -> BidiGrowVec<T> {
    bidigrowvec! {
...
fn helper_build_10x10<T: Testable>() -> BidiGrowVec<T> {
    bidigrowvec! {
...
fn helper_build_5x1<T: Testable>() -> BidiGrowVec<T> {
    bidigrowvec! {
        [T::new(11), T::new(12), T::new(13), T::new(14), T::new(15)],
    }
}

fn helper_build_1x5<T: Testable>() -> BidiGrowVec<T> {
    bidigrowvec! {
        [T::new(11)],
        [T::new(21)],
        [T::new(31)],
...
fn assert_layout<T: Testable>(
    ...
    if items_differ || len_differs || height_differs || width_differs {
        println!("BidiGrowVecs are different:");
        println!(
    ...
    BidiGrowVec::<T>::from_vec(v, vwidth).unwrap()
}
...
fn truncate_90x18<T: Testable>() {
    let mut v = BidiGrowVec::with_size_func_xy(90, 18, |x, y| T::new((y * 100 + x) as i32));
...
fn macro_invocation_arrayed<T: Testable>() {
    let v = bidigrowvec![T::new(1), T::new(2), T::new(3), T::new(4), T::new(5), T::new(6); 3];
...
fn bidiview_interop_basics<T: Testable>() {
    ...
    let v3 = BidiGrowVec::from_view(v1).unwrap();
...
fn bidiview_interop_clone<T: Testable + Clone>() {
    let v1 = helper_build_10x10::<T>();
    let v2 = BidiGrowVec::from_view(&v1 as &dyn BidiView<Output = T>).unwrap();
    assert_view_eq_views(&v1, &v2);
}
...
fn bidiview_interop_cut1<T: Testable + Clone>() {
    ...
    let v2 = BidiGrowVec::from_view_cut(&v1 as &dyn BidiView<Output = T>, &cut).unwrap();
...
fn bidiview_interop_cut2<T: Testable>() {
    let v1 = helper_build_10x10::<T>();
    let cut = BidiRect::new(0, 0, 3, 3);
    let v2 = BidiGrowVec::from_view_cut(v1, &cut).unwrap();
    assert_view_eq_views(
        &bidigrowvec! {
            [0, 1, 2],
            [10, 11, 12],
